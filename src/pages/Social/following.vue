...
<page-query>
query ($page: Int){
  allFollowing(limit:100, perPage: 9, page: $page) @paginate {
    pageInfo {
      currentPage
      totalPages
      perPage
...
export default {
  ...
  computed: {
    ...
    pageInfo() {
      return this.$page.allFollowing.pageInfo
    },
    followersTotal() {
      return this.$page.allFollowers.totalCount
    }
  },
  methods: {
    changeTab(tab) {
    ...
    changePage(page) {
      if (page == 1) {
        window.location.href = `/social/following`
      } else {
        window.location.href = `/social/following/${page}`
      }
    }
  }
}
